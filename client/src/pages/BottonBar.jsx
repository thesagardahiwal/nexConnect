...
const BottomBar = ({width}) => {
  ...
  const handleFileShare = () => {
    if(file) {
      ...
      reader.onload = (e) => {
        ...
        socket.emit("share-file", {filename: file.name, content: fileContent, roomId: roomID.roomId})
      }
  ...
  const handleSubmit = (e) => {
    ...
    if(socket.id) {
      if(message) {
        socket.emit("group-message", {message: message, id:socket.id, roomId:roomID.roomId});
        setMessage("");
      };
      if(file) {
